...
import { 
  Heart, MessageCircle, Plus, Leaf, HelpCircle, Camera, BookOpen, Send, X, 
...
const CommunityContent = () => {
  ...
  const [subTab, setSubTab] = useState<'foros' | 'guias'>('foros');
  ...
  const { data: profiles = {} } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data } = await supabase.from('profiles').select('user_id, name, avatar_url, user_type').in('user_id', userIds);
  ...
  const { data: comments = [] } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data } = await supabase.from('post_comments').select('*').eq('post_id', selectedPost.id).order('vote_count', { ascending: false });
  ...
  const { data: commentProfiles = {} } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data } = await supabase.from('profiles').select('user_id, name, avatar_url').in('user_id', commentUserIds);
  ...
  const { data: userVotes = [] } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data } = await supabase.from('comment_votes').select('comment_id').eq('user_id', user.id).in('comment_id', commentIds);
  ...
  const createPostMutation = useMutation({
    mutationFn: async () => {
      ...
      const { error } = await supabase.from('community_posts').insert({
        user_id: user.id, title: newPost.title.trim(), content: newPost.content.trim(), category: newPost.category, tags,
      });
      if (error) throw error;
    },
    onSuccess: () => {
  ...
  const addCommentMutation = useMutation({
    mutationFn: async () => {
      ...
      const { error } = await supabase.from('post_comments').insert({ post_id: selectedPost.id, user_id: user.id, content: newComment.trim() });
      if (error) throw error;
    },
    onSuccess: () => {
  ...
  const voteCommentMutation = useMutation({
    mutationFn: async (commentId: string) => {
      ...
      if (hasVoted) { await supabase.from('comment_votes').delete().eq('comment_id', commentId).eq('user_id', user.id); }
      else { await supabase.from('comment_votes').insert({ comment_id: commentId, user_id: user.id }); }
    },
    onSuccess: () => {
  ...
  const markBestAnswerMutation = useMutation({
    mutationFn: async (commentId: string) => {
      if (!user || !selectedPost) throw new Error('Missing data');
      if (selectedPost.user_id !== user.id) throw new Error('Only post author');
    ...
    return (
      <div className="flex flex-col">
        <div className="px-4 py-3 flex items-center gap-3">
          ...
          <h2 className="text-foreground text-base font-bold flex-1">{selectedGuide.title}</h2>
        </div>
  ...
  return (
    <div className="flex flex-col flex-1">
      {/* Sub-tabs: Foros / Guías + New post button */}
      <div className="sticky top-[100px] z-10 glass-strong">
        <div className="flex items-center px-4 gap-1">
          <button
            onClick={() => setSubTab('foros')}
            className={cn(
              'flex-1 py-2 text-xs font-bold text-center transition-all',
              subTab === 'foros' ? 'text-primary border-b-2 border-primary' : 'text-muted-foreground hover:text-foreground'
            )}
          >
            💬 Foros
          </button>
          <button
            onClick={() => setSubTab('guias')}
            className={cn(
              'flex-1 py-2 text-xs font-bold text-center transition-all',
              subTab === 'guias' ? 'text-primary border-b-2 border-primary' : 'text-muted-foreground hover:text-foreground'
            )}
          >
            📖 Guías
          </button>
          {subTab === 'foros' && (
            <Button onClick={() => setShowNewPost(true)} size="sm" className="bg-primary text-primary-foreground rounded-full h-7 px-3 text-[11px] font-bold ml-2">
        ...
        {subTab === 'foros' ? (
          <>
            <div className="px-4 pt-2">
              <div className="flex items-center rounded-full bg-muted/80 border border-border/50 px-3 gap-2 h-8">
                <Search size={13} className="text-muted-foreground shrink-0" />
                <input
                  type="text"
                  placeholder="Buscar hilos, etiquetas..."
                  value={searchQuery}
                  onChange={(e) => setSearchQuery(e.target.value)}
                  className="bg-transparent text-foreground text-xs placeholder:text-muted-foreground w-full outline-none"
                />
                {searchQuery && (
                  <button onClick={() => setSearchQuery('')} className="text-muted-foreground"><X size={13} /></button>
                )}
              </div>
            </div>
              ...
              {FORUM_CATEGORIES.map((cat) => {
                ...
                return (
                  <button key={cat.value} onClick={() => setActiveCategory(cat.value)}
                    className={cn('flex shrink-0 items-center gap-1.5 rounded-full px-3 py-1.5 transition-all duration-200',
                      isActive ? 'bg-primary text-primary-foreground shadow-md shadow-primary/25' : 'bg-muted/60 text-muted-foreground hover:bg-muted border border-border/30'
                    )}
                  >
                    <Icon size={11} strokeWidth={isActive ? 2.5 : 2} />
                    <span className={cn('text-[10px] whitespace-nowrap', isActive ? 'font-bold' : 'font-medium')}>{cat.label}</span>
                  </button>
                );
              })}
            </div>
          ...
          <div className="flex gap-2 px-4 py-2 overflow-x-auto scrollbar-hide">
            {GUIDE_CATEGORIES.map((cat) => {
              ...
              return (
                <button key={cat.value} onClick={() => setGuideCategory(cat.value)}
                  className={cn('flex shrink-0 items-center gap-1.5 rounded-full px-3.5 py-1.5 transition-all duration-200',
                    isActive ? 'bg-primary text-primary-foreground shadow-md shadow-primary/25' : 'bg-muted/60 text-muted-foreground hover:bg-muted border border-border/30'
                  )}
                >
        ...
        {subTab === 'foros' ? (
          <>
            {isLoading ? (
              ...
              <div className="text-center py-16">
                ...
                <h3 className="text-foreground text-sm font-bold mb-1">{searchQuery ? 'Sin resultados' : 'No hay hilos aún'}</h3>
                <p className="text-muted-foreground text-xs mb-4">{searchQuery ? 'Prueba con otros términos' : '¡Sé el primero en abrir un hilo!'}</p>
                {!searchQuery && (
                  ...
                  <div key={post.id} className="bg-card rounded-xl p-4 border border-border/40 space-y-3">
                    <div className="flex items-center gap-2.5">
                      <Link to={`/user-profile/${post.user_id}`}>
                        <Avatar className="h-9 w-9">
                          ...
                          <AvatarFallback className="bg-muted text-muted-foreground text-xs">{(profile?.name || 'U')[0]}</AvatarFallback>
                        </Avatar>
                      </Link>
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-1.5">
                          <Link to={`/user-profile/${post.user_id}`} className="text-foreground text-sm font-semibold truncate hover:underline">{profile?.name || 'Usuario'}</Link>
                      ...
                      {catInfo && <span className={cn('px-2 py-0.5 rounded-full text-[10px] font-bold', catInfo.color)}>{catInfo.label}</span>}
                    </div>
                    ...
                    {post.tags && post.tags.length > 0 && (
                      <div className="flex gap-1.5 flex-wrap">
                        {post.tags.map((tag, i) => (
                          <button key={i} onClick={() => setSearchQuery(tag)} className="text-[10px] font-medium text-primary bg-primary/10 px-2 py-0.5 rounded-full hover:bg-primary/20 transition-colors">{tag}</button>
                        ))}
                      </div>
                    )}
                    {post.images && post.images.length > 0 && (
                      <div className="flex gap-2 overflow-x-auto scrollbar-hide">
                        {post.images.map((img, i) => (<img key={i} src={img} alt="" className="h-32 rounded-lg object-cover" />))}
                      </div>
                    )}
                    {post.best_answer_id && (
                      <div className="flex items-center gap-1.5 text-emerald-400 text-[10px] font-bold"><Award size={12} /><span>Tiene mejor respuesta</span></div>
                    )}
                    <div className="flex items-center gap-4 pt-1">
                      <button onClick={() => toggleLikeMutation.mutate(post.id)} className={cn('flex items-center gap-1.5 text-xs transition-colors', isLiked ? 'text-red-400' : 'text-muted-foreground hover:text-red-400')}>
                        <Heart size={16} className={isLiked ? 'fill-current' : ''} /><span>{post.likes_count}</span>
                      </button>
                      <button onClick={() => setSelectedPost(post)} className="flex items-center gap-1.5 text-xs text-muted-foreground hover:text-primary transition-colors">
          ...
          <>
            {filteredGuides.map((guide) => (
              <button key={guide.id} onClick={() => setSelectedGuide(guide)} className="w-full bg-card rounded-xl p-4 border border-border/40 flex items-center gap-3 text-left hover:bg-muted/50 transition-colors">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-1 mt-1.5 text-primary text-[10px] font-semibold"><span>{guide.tips.length} consejos</span></div>
                </div>
          ...
          <div className="flex flex-col h-full">
            <SheetHeader className="px-4 pt-4 pb-3 border-b border-border/50">
              <div className="flex items-center justify-between">
                ...
                <Button onClick={() => createPostMutation.mutate()} disabled={!newPost.title.trim() || !newPost.content.trim() || createPostMutation.isPending} size="sm" className="bg-primary text-primary-foreground rounded-full h-8 px-4 text-xs font-bold">
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4">
              <div>
                ...
                <div className="flex gap-2 overflow-x-auto scrollbar-hide">
                  {FORUM_CATEGORIES.filter(c => c.value !== 'all').map((cat) => {
                    const Icon = cat.icon;
                    return (
                      <button key={cat.value} onClick={() => setNewPost(prev => ({ ...prev, category: cat.value }))}
                        className={cn('flex shrink-0 items-center gap-1.5 rounded-full px-3 py-1.5 text-[11px] transition-all',
                          newPost.category === cat.value ? 'bg-primary text-primary-foreground font-bold' : 'bg-muted text-muted-foreground border border-border/30'
                        )}
                      >
              ...
              <Input placeholder="Título del hilo" value={newPost.title} onChange={(e) => setNewPost(prev => ({ ...prev, title: e.target.value }))} className="bg-muted border-border/50 text-foreground placeholder:text-muted-foreground rounded-xl h-12" maxLength={100} />
              <Textarea placeholder="Describe tu duda, comparte tu experiencia o pide ayuda..." value={newPost.content} onChange={(e) => setNewPost(prev => ({ ...prev, content: e.target.value }))} className="bg-muted border-border/50 text-foreground placeholder:text-muted-foreground rounded-xl min-h-[160px] resize-none" maxLength={2000} />
              <div>
                <label className="text-foreground text-xs font-semibold mb-2 block"><Tag size={12} className="inline mr-1" />Etiquetas (separadas por coma)</label>
                <Input placeholder="cactus, interior, riego..." value={newPost.tags} onChange={(e) => setNewPost(prev => ({ ...prev, tags: e.target.value }))} className="bg-muted border-border/50 text-foreground placeholder:text-muted-foreground rounded-xl h-10" />
                <div className="flex gap-1.5 flex-wrap mt-2">
                  {POPULAR_TAGS.slice(0, 6).map(tag => (
                    <button key={tag} onClick={() => {
                      ...
                      if (!current.includes(tagClean)) { setNewPost(prev => ({ ...prev, tags: [...current, tagClean].filter(Boolean).join(', ') })); }
                    }} className="text-[10px] font-medium text-primary bg-primary/10 px-2 py-0.5 rounded-full hover:bg-primary/20">{tag}</button>
                  ))}
                </div>
              </div>
            </div>
      ...
      <Sheet open={!!selectedPost} onOpenChange={(open) => !open && setSelectedPost(null)}>
        <SheetContent side="bottom" className="h-[85vh] bg-card border-t border-border rounded-t-2xl p-0">
          <div className="flex flex-col h-full">
            <SheetHeader className="px-4 pt-4 pb-3 border-b border-border/50">
              <SheetTitle className="text-foreground text-sm font-bold text-left">{selectedPost?.title}</SheetTitle>
              {selectedPost && <p className="text-muted-foreground text-xs text-left mt-1 line-clamp-3">{selectedPost.content}</p>}
              {selectedPost?.tags && selectedPost.tags.length > 0 && (
                <div className="flex gap-1.5 flex-wrap mt-2">
                  {selectedPost.tags.map((tag, i) => (<span key={i} className="text-[10px] font-medium text-primary bg-primary/10 px-2 py-0.5 rounded-full">{tag}</span>))}
                </div>
              )}
            </SheetHeader>
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-3">
              {comments.length === 0 ? (
                ...
                comments.map((comment) => {
                  ...
                  return (
                    <div key={comment.id} className={cn('rounded-xl p-3 border transition-all', isBestAnswer ? 'border-emerald-500/50 bg-emerald-500/5' : 'border-border/30 bg-muted/20')}>
                      {isBestAnswer && <div className="flex items-center gap-1.5 text-emerald-400 text-[10px] font-bold mb-2"><Award size={12} /><span>Mejor respuesta</span></div>}
                      <div className="flex gap-2.5">
                        <div className="flex flex-col items-center gap-0.5 pt-1">
                          <button onClick={() => voteCommentMutation.mutate(comment.id)} className={cn('p-1 rounded transition-colors', hasVoted ? 'text-primary' : 'text-muted-foreground hover:text-primary')}>
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2">
                            <Avatar className="h-6 w-6 shrink-0">
                              ...
                              <AvatarFallback className="bg-muted text-muted-foreground text-[9px]">{(cp?.name || 'U')[0]}</AvatarFallback>
                            </Avatar>
                          ...
                          {isPostAuthor && (
                            <button onClick={() => markBestAnswerMutation.mutate(comment.id)} className={cn('flex items-center gap-1 mt-2 text-[10px] font-semibold transition-colors', isBestAnswer ? 'text-emerald-400' : 'text-muted-foreground hover:text-emerald-400')}>
                              <Award size={11} />{isBestAnswer ? 'Quitar mejor respuesta' : 'Marcar como mejor respuesta'}
                            </button>
                          )}
                        </div>
                      </div>
            ...
            <div className="p-3 border-t border-border/50 flex gap-2">
              <Input placeholder="Escribe tu respuesta..." value={newComment} onChange={(e) => setNewComment(e.target.value)} className="bg-muted border-border/50 text-foreground placeholder:text-muted-foreground rounded-full h-10 text-xs" maxLength={500}
                onKeyDown={(e) => { if (e.key === 'Enter' && newComment.trim()) addCommentMutation.mutate(); }}
              />
              <Button onClick={() => newComment.trim() && addCommentMutation.mutate()} disabled={!newComment.trim() || addCommentMutation.isPending} size="sm" className="bg-primary text-primary-foreground rounded-full h-10 w-10 p-0 shrink-0">
                <Send size={16} />
              </Button>
            </div>
          </div>
